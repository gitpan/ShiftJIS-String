...
For example,
C<0x41> as C<'A'> is lesser than C<0xB1> as C<'ｱ'>.
C<0xB1> as C<'ｱ'> is lesser than C<0x8341> as C<'ア'>.
C<0x8341> as C<'ア'> is lesser than C<0x8383> as C<'ャ'>.
C<0x8383> as C<'ャ'> is lesser than C<0x83B1> as C<'Τ'> (greek Tau).

B<Caveat!> Compare the 2nd and the 4th examples.
...
B<Note:> C<strEQ> is an expensive equivalent of the C<CORE>'s C<eq> operator.

=item C<strNE(LEFT-STRING, RIGHT-STRING)>

Returns a boolean whether C<LEFT-STRING> is not equal to C<RIGHT-STRING>.

B<Note:> C<strNE> is an expensive equivalent of the C<CORE>'s C<ne> operator.
